...
class Ion_auth_model extends CI_Model
{
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$sql="Select id_persona from dp_auth_users where carrera_id='$carrera_id' and id='$id' ";
	...
	**/
	...
	{
       $sql="SELECT id as persona_id, ci, expedido, nombre, paterno, materno, fecha_nac, genero FROM base_upea.persona  WHERE ci = $ci";
        $query = $this->db->query($sql);
		if( $query->num_rows()>0 )
			return $query->row();
		else        
	...
	/*public function validar_usuario_kardex_carrera_activa($id_usuario,$id_carrera){

		$user_carrera=$this->db->query("SELECT
		dp_auth_users_groups.group_id,
		dp_auth_users.id_persona,
		dp_auth_groups.name,
		dp_auth_users_groups.user_id,
		dp_auth_users_groups.id AS id_grupos,
		dp_auth_users.carrera_id,
		dp_auth_users.id as iduser,
		dp_auth_users.active
		FROM dp_auth_users_groups
		INNER JOIN dp_auth_groups ON dp_auth_users_groups.group_id = dp_auth_groups.id
		INNER JOIN dp_auth_users ON dp_auth_users_groups.user_id = dp_auth_users.id
		WHERE dp_auth_groups.name='kardixta'
		AND dp_auth_users.id='$id_usuario'
		AND dp_auth_users.carrera_id='$id_carrera'
		AND dp_auth_users.active='1' ")->row();
		if ($user_carrera) {
			$vista=$this->db->query("SELECT * FROM base_upea.vista_personal_kardex_carrera as base_upea
			WHERE base_upea .id='$user_carrera->id_persona'
			AND base_upea .carrera_id='$id_carrera' ")->row();

			if ($vista) {
				return true;
			}else{
				// EXCEPTO USUARIO 1
				if($user_carrera->iduser=='1' || $user_carrera->iduser=='2'){
					return true;
				}else{
					// kardex activo pero no existe vista
					$veri_grupo=$this->db->query("SELECT
					dp_auth_users_groups.id AS id_grupos,
					dp_auth_groups.name
					FROM dp_auth_users_groups
					INNER JOIN dp_auth_groups ON dp_auth_users_groups.group_id = dp_auth_groups.id
					WHERE dp_auth_groups.name='kardixta'
					AND dp_auth_users_groups.user_id = '$id_usuario' ")->row();
					if ($veri_grupo) { 
						// echo "eliminar grupo";exit();
						// eliminar grupo
						$this->db->where("id",$veri_grupo->id_grupos);
						$this->db->delete("dp_auth_users_groups");
					}else{
						// desactivar usuario
						$obj=array('active'=>'0');
						$this->db->where("id",$user_carrera->iduser);
	...
	public function reset_usuario($id_usuario){
		return $this->db->query("SELECT  * FROM dp_auth_users WHERE id='$id_usuario' ")->row();
	}
	...
	public function guardar_nuevo_usuario($nombre,$apellido,$id_carrera,$id_tipo_usuario,$name,$pass,$idpersona){
		$pas=$this->hash_password($pass);
...
public function guardar_editar_usuario($id_usuario1,$id_tipo_usuario1){
	// $obj=array(
	// 	'first_name'=>$nombre,
	// 	'last_name'=>$apellido
	// );
